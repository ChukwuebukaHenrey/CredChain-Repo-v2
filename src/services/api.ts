...
export async function getCandidate(id: string)         { return USE_MOCK ? mock.candidate         : get(`/candidates/${id}`) }
...
export async function signup(payload: any) {
  if (USE_MOCK) {
    return {
      ...
      user: {
        id: "user-" + Date.now(),
        name: payload.name || payload.institution || payload.company || "New User",
        email: payload.email,
